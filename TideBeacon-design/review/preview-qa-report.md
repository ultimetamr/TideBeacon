...
> artifactRevision: 6｜scope=`web_design_validation_only`｜source interaction@11, visual@6, design-system-review@9 pass｜preview.html@6
> status: `pass`（independent PQA R6）
...
|S7|`#app[data-state=S7]`|X10/X07|C6|完成文案与可选记录|pass|
...
|X05|`[data-action=X05]`|继续|S4→S3；无跳时；normal 400ms / performance 600ms 淡入|pausedAccum 重基准；audioState 精确标识 400/600ms|pass|
|X06|`[data-action=X06],[data-system-back]`|退出/Back|S4→S5|pending=exit|pass|
|X07|`[data-dialog=confirm]`|确认退出|S5→S7|完成页可见|pass|
...
|C7 pendingIntent|`[data-binding=pendingIntent]`|X02/X06/X08；fallback/error|start/exit/restart；非正常数据取消并返回|标题/data-variant 实际变化；dataMode 非 normal 清空 pending 并返回 S1/S4|pass|
...
|Resume rebase|X05|no jump + smooth audio return|pausedAccum adds pause span; normal 400ms, performance fallback 600ms|pass|
...
PM@6 R1–R15 map to S0–S7, X01–X12 and C1–C7. Exact first-use and completion copy are rendered. Durations 2/4/6, scenes sea/cloud/dune, pattern selection, pause/exit/restart, optional local record, ReduceMotion, weak cycle particles and controller Back are all triggerable. Screenshot capture, Android/PICO runtime and device audio remain downstream; this package intentionally generates no Android code.
...
Independent R6 rebuilt states 8, transitions 12, render elements 33, bindings 30, variants 27, component states 63, stacking 7, responsive/motion 4；all manifest diffs are zero and no active finding remains. `minimumCompletenessGate=pass`.

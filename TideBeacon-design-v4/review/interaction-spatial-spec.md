...
> artifactRevision: 9｜v4 independently rebuilt｜active pm@2 uxr@1 visual@3
...
|---|---|---|---|---|---|---|---|
| T1 理解光语 | 初次用户/稳定引导界面 | R3,E-U1 | 说出扩张=吸气、收回=呼气 | 无法跟随 | 首次 | none | ≤10s |
| T2 配置航程 | 用户/选择 | R4–R6,C1–C3 | duration+scene+pattern | 错时长/不适偏好 | 每次 | T1 | 每项≤5s |
| T3 同意入海 | 用户/开始前 | E-P1,QC-WINDOW | 明确确认“开始沉浸练习”或留在选择 | 非预期进入沉浸状态 | 每次 | T2 | 1 action |
...
| T5c 用户退出 | 用户/暂停焦点或 system back | R10/R12,E-P1 | 确认退出到稳定选择状态或关闭（明确不进入完成），或取消 | 误退出/误作完成/无稳定出口 | 偶发 | T4/T5a | confirm |
...
| T5a 用户暂停/恢复 | time; control plane | 暂停层只说明控制优先级，非空间净增益 | 2D 单平面暂停卡足以表达冻结快照和恢复；Stage 中也只用单平面 | R10/R11,E-S3 | Low |
| T5b 重新开始 | decision confirmation | 仅是破坏性决策，无方向/深度收益 | 2D 确认对话框完全足够，且更清楚地保护当前进度 | R10,QC-RISK | Low |
| T5c 退出 | decision confirmation; stable exit | 仅是安全出口，空间化会增加误触面 | 2D 确认对话框足够；确认后返回选择/关闭，不进完成 | R10/R12,E-P1 | Low |
| T5d 系统挂起/恢复 | time; lifecycle | 这是无用户布局的生命周期事件，无空间价值 | 2D 或 Stage 均应保存同一 elapsed 快照；回前台保持 paused，只在用户明确恢复后 400ms 淡入 | R11/R14,E-S3 | None |
...
| B | 1 | 1 | 3p | 1 | 3p | 1 | 1 | 3p | 14 | Eff:T2 低空间价值且弧带增路径=1；Spatial:T2=none/2D更高效=1；Comfort:B 单静态弧带+无自动移动达3、真机未验故3p；Domain:B 未定义完成与中断路径，漏核心故1；Safety:B 单焦点/无自动移动+QC-RISK达3但未验故3p；Access:B 仅有单一手柄选择，无文本缩放/ReduceMotion/焦点闭环，故1；Feasible:G-P1 仅未知缺口故1；Unique:C1/C2 opportunity仅支持3p / Rejected |
| C | 1 | 3p | 3p | 1 | 3p | 1 | 1 | 3p | 16 | Eff:T1/T2 低价值却强制 Full Space=1；Spatial:T4 High-provisional 但 T1/T2 Low，故3p；Comfort:C 固定视点+crossfade达3、真机未验故3p；Domain:C 未定义 pause/resume/restart/exit，漏核心故1；Safety:C 固定相机+互斥可见+手柄确认达3但未验故3p；Access:C 仅有手柄确认/固定视点，无文本缩放/ReduceMotion/焦点闭环，故1；Feasible:G-P1 仅 SDK 未知故1；Unique:OPP-1 机会层仅支持3p / Rejected |

评分插值规则：2=达到1锚点，并具有一项独立3分证据但未满足全部3分条件；4=完整达到3锚点，并具有一项独立5分证据但未满足全部5分条件。`p` 表示 provisional，上限4。A Domain=4：完整领域状态+统一时间轴达到3，并有四相位/循环边界的5分证据但未真人验证；A Safety=4p：硬边界/ReduceMotion达到3，并有稳定退出与冻结的5分证据，但设备舒适度未验证。
...
| S5 ExitConfirm | T5c | C7 ConsentDialog overlay/PausePanel | exit intent | exit request→S1 | cancel→S4 |
...
| X07 | S5→S1 | user.confirmExit | closeStage,discardActive,showChooser | yes |
...
S0→S1→S2(confirm)→S3↔S4；S5 确认退出回 S1（不显示完成）；S6 重开回 S3；仅 X10 durationReached 进入 S7。系统挂起总是 S4；回前台不自动播放。
...
v4 Stage11 verifiedAt 2026-08-12T21:33:04+08:00: controller fallback, destructive confirmation, same-elapsed motion/audio/countdown derivation, Reduce Motion, 400ms normal/performance resume and data-display fallbacks were reconciled with visual@3.
...
| Resume audio | X05/avoid pop | 400ms gain 0→target; position unchanged | same | 400ms linear |
...
v4 Stage 09 verifiedAt 2026-08-12T21:32:10+08:00: A resolves BeaconWindow Planar in Shared, TideStage in Full, and PausePanel as single focus layer; container legality, explicit entry, stable exit, sizing/FOV and attachment None/InlineControl comparison were verified against active visual@2 and sizing methodology.
v4 Stage 10 verifiedAt 2026-08-12T21:32:38+08:00: S0–S7 composition, L0/L1/L3/L4/L7 layouts, task/data/frequency/spatial derivation, responsive reflow, focus order and single-primary-focus rules align with Stage09 default/min/max geometry.

v4 Stage 05 verifiedAt 2026-08-12T21:20:27+08:00: T1–T6 each retain decision output, failure consequence, frequency, dependency, exception and stable exit against source@1/pm@2/uxr@1.
v4 Stage 06 verifiedAt 2026-08-12T21:20:50+08:00: A「远航灯标」/B「静态环形仪表舱」/C「地标序列」在信息模型、容器、空间化与路径上实质不同；八维 1/2/3/4/5 锚点、逐格 evidenceRefs、2D 反事实与选 A 理由基于 active evidence 核验。
...
| Historical pre-Stage10 checklist | invalidated by v4 Stage10–11 rebuild | invalidated |

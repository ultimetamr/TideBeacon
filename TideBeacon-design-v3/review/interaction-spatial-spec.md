...
> artifactRevision: 17｜v3 clean baseline｜active pm@11 uxr@8 visual@10
...
| T5c 用户退出 | 用户/暂停焦点或 system back | R10/R12,E-P1 | 确认退出到稳定完成/选择状态或取消 | 误退出/无稳定出口 | 偶发 | T4/T5a | confirm |
...
v3 Stage 09 decision: after behavioral deferral, A resolves BeaconWindow Planar in Shared, TideStage in Full, and PausePanel as single focus layer; container legality, explicit entry, stable exit, sizing/FOV and attachment None are revalidated.
v3 Stage 10 revalidation: S0–S7 composition, L0–L5 layouts, priority, responsive reflow, focus order and single-primary-focus rules rebuilt from the Stage09 decision.

v3 Stage 05 revalidation: T1–T6 each retain decision output, failure consequence, exception and stable exit；prior receipts not reused.
v3 Stage 06 revalidation: A「远航灯标」/B「静态环形仪表舱」/C「地标序列」remain substantially different；eight-dimension 1/2/3/4/5 semantics and per-cell evidenceRefs were rebuilt, A selected without template reuse.
...
| Historical pre-Stage10 checklist | invalidated by v3 Stage10–11 rebuild | invalidated |
